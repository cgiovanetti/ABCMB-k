"""Inspect lna_start for each k mode."""
...
def main():
    ...
    print(f"BG.lna_transfer_start = {float(bg.lna_transfer_start)}", flush=True)

    @eqx.filter_jit
    def lna_starts(PE, BG, params):
        def per_k(k):
            ls = PE.get_starting_time(k, (BG, params))
            ls = jnp.minimum(ls, -10.)
            return ls
        return vmap(per_k)(PE.k_axis_perturbations)

    ls = np.asarray(lna_starts(PE, bg, full_p))
    k = np.asarray(PE.k_axis_perturbations)
    for i in [0, 1, 5, 50, 99, 100, 150, 152, 156, 184, 200, 300, 400, 491]:
        print(f"  k_axis[{i:>3}]={k[i]:.4e}  lna_start={ls[i]:.4f}", flush=True)
