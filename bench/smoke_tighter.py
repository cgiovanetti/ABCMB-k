"""Tighten rtol/atol and see if pure single-k agrees with vmap."""
...
def main():
    # Tighter rtol/atol for large-k:
    for rtol_large, atol_large in [(1e-4, 1e-6), (1e-6, 1e-9), (1e-7, 1e-10)]:
        model = Model(
            ...
            rtol_large_k_PE=rtol_large,
            atol_large_k_PE=atol_large,
            max_steps_PE=32768,
        )
        PE = model.PE
        ...
        print(f"\n=== rtol_large={rtol_large} atol_large={atol_large} ===",
              flush=True)
        sample_idx = [155, 184, 199]
        pure_results = {}
        ...
        for idx in sample_idx:
            ...
            print(f"  idx={idx} pure={norm:.3e}  "
                  f"full_vs_pure={d_full:.2e}  chunk_vs_pure={d_chunk:.2e}  "
                  f"full_vs_chunk={d_full_chunk:.2e}", flush=True)
