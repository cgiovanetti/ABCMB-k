...
with CLASS to $\sim0.2\%$ on TT at $\Lmax=2508$ --- comfortably inside the
published $1\%$ accuracy gate. The solver is the \emph{forward
...
\end{quote}
(Here ``$\pm\,1/2\,\sigma$'' means the $\pm1\sigma$ and $\pm2\sigma$
confidence intervals.) The \textbf{competitive bar} is concrete: a CLASS forward model driven by
...
coverage --- is described in Section~\ref{sec:coverage}. The batched
coverage validator is \emph{built and GPU-checked} (\file{scan/wilks.py});
as of 2026-06-29 the production $500$-mock $\Lambda$CDM and
$\Lambda\mathrm{CDM}+\Neff$ runs are \emph{queued} (no coverage table
written yet), and the preliminary numbers in Section~\ref{sec:coverage} are
log-scraped from a partial pass, not from a saved table.
...
\code{expmkappa\_tab} keystone, the sharding helper) were developed on
branch \code{perk-perf} and are now merged into \code{main}, the canonical
branch to check out. Hard constraints honoured throughout, set by the user:
...
replaced by the MLE Fisher errors (defined in Section~\ref{sec:profvsmarg})
when the MLE pre-pass is enabled (Section~\ref{sec:mle}). \\
...
likelihood ($\nu$ here is nuisances, \emph{not} neutrinos --- the effective
neutrino number is $\Neff$); $\nu\in\R^{21}$ for full Planck \code{plik}, or
the single calibration $A_{\mathrm{planck}}$ for \code{plik\_lite}. \\
...
$\sigma_1$ & The $\dchisq=1$ profile interval half-width for a POI (the
$1\sigma$ confidence half-width --- \emph{not} a component of the
prior-scale array $\sigma$ above); the quantity the early-stop monitors. \\
...
fast/slow split). \\
$w_i,\ \bm{W}$ & $w_i$ the normalised SMC weight of particle $i$ (the
effective sample size is $\mathrm{ESS}=1/\sum_i w_i^2$); $\bm W$ the
cumulative (un-normalised) importance weights, so $\log\bm W$ is the
running log-weight array. \\
$Z$ & Bayesian \emph{evidence} $Z=\int L\,\pi\,\mathrm d\theta\,\mathrm d\nu$,
the prior-averaged likelihood (the normaliser of the posterior and the
basis of model comparison; Section~\ref{sec:smcbg}); $\log Z$ is accumulated
from the SMC normalising constants. \\
...
First, the vocabulary this section (and the rest of the manual) leans on.
The \emph{maximum-likelihood estimate} (MLE) is the parameter point that
maximises $L$, equivalently minimises $\chisq$. A \emph{prior}
$\pi(\theta,\nu)$ is a probability density encoding what is assumed about
the parameters \emph{before} seeing data; the \emph{posterior}
$p(\theta,\nu\mid\mathcal D)\propto L\,\pi$ is the corresponding density
\emph{after} conditioning on data $\mathcal D$. The \emph{Fisher
(information) matrix} is the Hessian of $\tfrac12\chisq=-\ln L$ at the
optimum; its inverse approximates the parameter covariance, so its
square-rooted diagonal gives the ``Fisher errors'' (used to size grids,
Section~\ref{sec:mle}) and its inverse preconditions the optimiser
(Section~\ref{sec:bfgs}). The ``Fisher regime'' is the neighbourhood of the
optimum where $\chisq$ is well approximated by this quadratic. Finally the
$\chisq_1$ distribution is the distribution of the square of a single
standard Gaussian: $68\%$ of its mass lies below $1$ and $95\%$ below
$3.84\approx4$, which is why the profile thresholds $\dchisq\le1$ and
$\le4$ define the $1\sigma$ and $2\sigma$ intervals
(Wilks' theorem, Section~\ref{sec:wilks}).
...
(a smooth change of POI variable maps its interval consistently).
...
different optimiser, because its objective (clipy --- the JAX-native
re-implementation of the Planck full-\code{plik} likelihood,
Section~\ref{sec:plikfull}) supplies a reliable first derivative but
\emph{not} a usable Hessian, and several nuisances sit against hard bounds.
The right tool is a \textbf{Spectral Projected
...
\end{itemize}
SPG converges like a bound-constrained L-BFGS-B (limited-memory
bound-constrained BFGS, the reference \code{scipy} solver) but uses only
first derivatives, which is exactly the regime the clipy foreground profile
lives in.
...
\end{equation}
(An \emph{importance weight} re-weights a particle drawn from one
distribution to represent another --- here the ratio of consecutive
tempered targets; as the weights become uneven the population represents
the target with fewer \emph{effective} particles, quantified by the
\emph{effective sample size} $\mathrm{ESS}=1/\sum_i w_i^2$, between $1$ and
$N$.)
...
the target we \textbf{resample} --- \emph{systematic resampling}: duplicate
high-weight particles, drop low-weight ones, reset the weights to uniform
--- to refresh the population, then \textbf{move} each particle with a few
random-walk \emph{Metropolis} steps (propose a perturbed particle, accept
it with the Metropolis probability, so the move leaves $\pi_\beta$
invariant), using a proposal covariance scaled by the Gelman--Roberts
optimal factor $2.38^2/d$ (optimal for a $d$-dimensional Gaussian target)
times the weighted particle covariance. The
...
Internally it runs three vmapped sub-stages with three device transfers
(two arrays out to the CPU backend, one back):
...
well under the $\sim0.1$ $\chisq$ solver roughness floor (the $k$-chunking
/ discretisation roughness, Section~\ref{sec:cert}). The profiled $\chisq$ is differentiable in $\Cl$
...
\emph{log}-probability surface $\ln P_\ell$ in $\Dl^{EE}$; the EE $\chisq$
is $-2\sum_\ell\ln P_\ell(\Dl^{EE})$, reading the tabulated $\ln P_\ell$ by
cubic interpolation (a single logarithm --- the table already holds
$\ln P_\ell$), with
...
inverse of the positive-definite-regularised (PD) co-profiled submatrix of the $D\times D$
...
$\ginf$ floors at $\sim1$--$7$ (above $\mathrm{GTOL}=0.03$; full-\code{plik},
with its SPG inner profile, sits at the upper end --- $\sim6.9$ measured on
$\tau$) because of a
...
interval to $0.0001\sigma$ --- a $\sim2.6\times$ wall-clock saving (less
than the $18/5$ iteration ratio, because the fixed JIT compile and the
always-run post-loop AD certificate are not saved). A
...
default for \code{plik\_lite} new-physics runs. \emph{Caveat for
full-\code{plik}:} the pre-pass's first jitted call is a single-point,
full-$D$ AD-through-SPG gradient --- a compile signature absent from the
warm batched-path cache, which at $\Lmax=2508$ monster-compiles and hangs
(it stalled the full-\code{plik} $\Neff$ run for two $10$~h walltimes with
zero iterations). For full-\code{plik} extensions, prefer a hand-recentered
config (grids placed on the joint MLE, e.g.\ \code{lcdm\_neff\_plikfull\_rc.py})
with \code{PA\_WARM\_FROM\_CEN=1} instead of the pre-pass. The $\sigma_1$
early-stop is additionally hardened so a single NaN/off-grid POI cannot run
the batch to the wall.

\subsection{Interval extraction}
\label{sec:intervals}

From each POI's $(\mathrm{grid},\hat\chisq)$ the deliverable intervals are
the $\dchisq=1$ and $\dchisq=4$ crossings, found by a shape-preserving
\textbf{PCHIP} (Piecewise Cubic Hermite Interpolating Polynomial)
interpolant of $\hat\chisq-\min\hat\chisq$
...
$\exp(-\chisq/2)$ is exactly the joint posterior
$p(\theta,\nu\mid\mathcal D)$; the $21$ nuisances are marginalised only when
the $\theta$-marginals are read off. The
...
Metropolis-within-Gibbs split (\emph{Gibbs}: cycle through disjoint blocks
of parameters, updating one block at a time with the others held fixed ---
here a slow cosmology block and a fast nuisance block), exactly the
fast/slow oversampling Cobaya uses, hand-rolled around \code{call\_batched}:
\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
...
nuisance-marginalised cosmology posterior sampled in the full
...
40\,GB node (and at $\Lmax=2508$+lensing OOMs even on 80\,GB: the
$\sim49$~GB in Table~\ref{tab:throughput} is the persistent footprint, and
transient XLA scratch pushes the peak past 80\,GB). Size $\Bloc$ for 40\,GB
unless you know the allocation is 80\,GB.
...
($\mathtt{JAX\_COMPILATION\_CACHE\_DIR}$) is set so warm restarts reuse the
cached XLA executables for the primal stages; note, though, that the
assembled staged-jvp gradient program is re-traced once per SLURM job (it
does not persist across jobs, Section~\ref{sec:gradcost}), so a resume
re-pays its $\sim5$~min build. The full-\code{plik} \code{"ad"} iteration
gradient is far heavier: its AD-through-SPG compile is $\sim53$~min at
$\Lmax=2508$, and the multi-GPU \emph{sharded} program is not captured by the
on-disk cache at all, so every job --- and every POI-slice rank --- re-pays
it; negligible against the multi-hour solves, but not amortised. Both tools
checkpoint per iteration/stage
and auto-resume, so
...
\paragraph{SMC}
The tempered SMC sampler is a consistent estimator of the posterior
(\emph{consistent}: its estimates converge to the true posterior as the
particle count grows; Del Moral, Doucet \& Jasra), with an \emph{unbiased}
evidence estimator (its expectation equals the true $Z$) from
...
frequentist interval and the Bayesian credible region (the posterior
analogue of a confidence interval --- the smallest set holding a given
posterior probability) would not coincide.
...
a physical edge ($\Delta\Neff\ge0$, $\sum m_\nu\ge0$, $f_{\mathrm{EDE}}\ge0$,
$r\ge0$) or strongly non-Gaussian directions. For those the honest tool is not Wilks
...
the coverage-mock study (Section~\ref{sec:coverage}), with the batched
validator \emph{built and GPU-validated}: it regenerates the $\dchisq$
distribution from mocks and tests it against $\chisq_1$ directly. A
preliminary, partial pass (4 of the 6 $\Lambda$CDM POIs; the medians are
log-scraped, not yet from a saved table, Section~\ref{sec:coverage}) puts
the per-mock median test statistic on the $\chisq_1$ median ($0.455$) for
every parameter it reached, so the $\Lambda$CDM intervals of
Section~\ref{sec:results} are corroborated as correctly covering rather than
merely Wilks-approximation intervals --- on three independent legs (the
profile-vs-SMC agreement, the near-Gaussian parameter structure, and the
direct mock coverage). The full $68\%/95\%$ coverage table, and the
Feldman--Cousins thresholds for the bounded directions where Wilks is
expected to fail, will come from the production runs (queued as of
2026-06-29; Section~\ref{sec:coverage}).
...
the $\dchisq<\ell$ interval iff $t<\ell$). (The coverage test uses the exact
$95\%$ threshold $3.84$; the deliverable $\dchisq\le4$ interval is the
nominal $2\sigma$, i.e.\ $95.45\%$.)
...
\textbf{shared-Jacobian Gauss--Newton} on the batch axis (\emph{Gauss--Newton}
approximates the Hessian of a least-squares objective by $\bm J^\top\bm J$,
the ``normal matrix,'' and solves $\bm J^\top\bm J\,\Delta\theta=-\bm
J^\top\bm r$ for the step, with $\bm J$ the model Jacobian and $\bm r$ the
residuals): the entire mock
...
$|0.33\sigma|$ of the SMC posterior mean --- the frequentist optimum and
the Bayesian posterior agree (mean and mode coincide here because the
marginals are near-Gaussian), which is the cross-check that matters. The
...
the largest co-profiled-Hessian condition number ($\sim159$) --- correctly
...
full-\code{plik} $\Neff$ run timed out (twice, $10$~h) and its
...
$h\approx0.666$, $\omega_c\approx0.1165$, $\Neff\approx2.83$, still drifting
in the unconverged salvaged checkpoint). The joint-MLE pre-pass was
\emph{not} the fix --- it monster-compiles at full-\code{plik} $\Lmax=2508$
(Section~\ref{sec:mle}); the definitive intervals instead come from a
hand-recentered, pre-pass-disabled re-run on the proven compile path
(config \code{lcdm\_neff\_plikfull\_rc.py}, \code{PA\_WARM\_FROM\_CEN=1},
$48$~h; launched 2026-06-29, job 55235706, in flight), which will produce
clean $1\sigma$ and $2\sigma$ bounds.
...
$\beta=1$ with a healthy effective sample size (ESS $=437$ and $512$ of $512$)
...
$h=0.682\pm0.011$ sits above the profile MLE $h\approx0.666$, with the profile
...
The \emph{Bayes factor} --- the ratio of model evidences,
$\exp\Delta\log Z\approx16$ --- ranks ``positive'' on the \emph{Jeffreys
scale} (the conventional ladder: barely / positive / strong / decisive),
consistent with $\Neff$ being within $1\sigma$ of $3.044$. This number is
\emph{indicative only}: the $+\Neff$ cosmology prior box was recentred and
widened on the joint MLE (Section~\ref{sec:mle}), so its prior volume ---
and hence the \emph{Occam factor} (the automatic penalty a Bayesian
evidence assigns for spreading prior probability over a larger volume: a
wider box dilutes the evidence even at fixed best fit) --- differs from the
$\Lambda$CDM run; a clean model comparison would re-run both under an
identical box. The plik-lite
...
Bayesian posterior mean to $|0.33\sigma|$ across all six $\Lambda$CDM
...
\item \textbf{Full-\code{plik} relaunch path} (single-POI dry run, job
55244960): the recentred \code{lcdm\_neff\_plikfull\_rc.py} grid with
\code{PA\_WARM\_FROM\_CEN=1} compiles ($\sim53$~min at $\Lmax=2508$),
iterates ($\sim19$~min/iter), and returns a finite $\tau$ interval
$0.0593^{+0.0046}_{-0.0068}$ agreeing with the full-\code{plik} SMC
($0.0632\pm0.0054$) to $\sim0.7\sigma$ --- validating the pre-pass-free
recentred path before the full $\Neff$ run.
\end{enumerate}
...
land well under the $22$~h CLASS+MH bar --- the definitive $\Neff$ timing
awaits the pre-pass-disabled, hand-recentered re-run
(Section~\ref{sec:mle}; job 55235706, $48$~h; a single-POI dry run of that
exact path measured $\sim53$~min compile $+\sim19$~min/iter, comfortably
inside budget) --- and it
...
# Base LCDM with a massless neutrino background:
params = {
...
$\mathcal A_s=\ln(10^{10}A_s)$ and convert), and
\code{model.add\_derived\_parameters(params)} returns the dict with all
derived keys filled if you want to inspect them. A \code{MassiveNeutrino} is
\emph{not} added by setting \code{N\_nu\_massive>0} alone --- the species
list is built from the $\Lambda$CDM defaults (a \emph{massless} neutrino)
plus whatever you pass in \code{user\_species}
(\file{abcmb/model\_specs.py}, \code{populate\_species}). A massive run
needs both the species \emph{and} its parameters:
\begin{lstlisting}
from abcmb import species
mnu_params = dict(params,
    N_nu_massive=1, T_nu_massive=0.71611, m_nu_massive=0.06)
model_mnu = Model(output_Cl=True, l_max=2508, lensing=True,
                  output_Pk=True, output_k_max=0.5,
                  l_max_g=12, l_max_pol_g=10, l_max_ur=17, l_max_ncdm=17,
                  user_species=(species.MassiveNeutrino,))
out_mnu = model_mnu(mnu_params)
\end{lstlisting}
...
\begin{lstlisting}
PA_CONFIG=scan/configs/lcdm_neff.py PA_POI_SLICE=1 PA_MLE_PREPASS=1 PA_TAG=_neff \
  sbatch --nodes=7 --ntasks-per-node=1 scan/profile_prod_ad.slurm
\end{lstlisting}
(\code{PA\_POI\_SLICE=1} is required to spread the $7$ POIs across the $7$
nodes --- without it all POIs run on rank~0 and the other six nodes idle.
The MLE pre-pass is safe here because \code{lcdm\_neff.py} is
\code{plik\_lite}; for a full-\code{plik} extension use a hand-recentered
config with \code{PA\_WARM\_FROM\_CEN=1} instead --- the pre-pass
monster-compiles at full-\code{plik} $\Lmax=2508$, Section~\ref{sec:mle}.)
...
$\sim2.6\times$ on the headline, Section~\ref{sec:earlystop}). For a model
you trust to be smooth you can loosen \code{PA\_SIGTOL} above $10^{-2}$ for a further
...
\emph{bounded} directions ($\Delta\Neff$, $\sum m_\nu$, $r$, $f_{\mathrm{EDE}}$),
...
$25$--$31$; the default is $25$, but the headline used walltime-driven
$11$-point grids) reduce the PCHIP spline's interpolation error and better
...
\begin{itemize}[leftmargin=1.5em,itemsep=1pt]
\item Increase the save-grid resolution: revert the engine default
...
\end{itemize}
The tension is real: two earlier rounds of memory work cut per-call memory
...
\begin{itemize}[leftmargin=1.5em,itemsep=1pt]
\item \textbf{Stream the perturbation table per $k$-chunk}
...
faster & high & stream PT per $k$-chunk; hybrid FD/AD \\
...
The batched validator is \emph{built and GPU-checked} (its shared-Jacobian
fitter is validated, Section~\ref{sec:coverage}); what is genuinely
\emph{planned} is the production output. As of 2026-06-29 the $\Lambda$CDM
and $\Lambda\mathrm{CDM}+\Neff$ $500$-mock runs are queued (no table on disk
yet); when they finish, what remains is collecting the final $68\%/95\%$
coverage tables and extending the construction to the bounded-parameter
directions ($\Delta\Neff$, $\sum m_\nu$, $r$, $f_{\mathrm{EDE}}$), where the
measured Feldman--Cousins thresholds --- not Wilks --- are the deliverable.

\paragraph{Other open items}
(These also appear in Section~\ref{sec:improve} as larger refactors.) LINX
under \code{vmap} for large-$B$ \code{bbn\_type="linx"} scans (LINX
currently runs per-cosmology in the eager \code{add\_derived\_parameters}
loop, fine at $\sim$ms/cosmo); and an intra-POI row-slice beyond the $6$-way
ceiling set by the $6$ base-$\Lambda$CDM POIs (needs an MPI gather).
...
\code{PA\_FD\_CHUNK}, \code{PA\_USE\_PLIK}, \code{PA\_HESS\_CACHE}, \code{PA\_FTOL}/\code{PA\_FTOL\_PATIENCE},
...
\midrule
\endhead
\code{PA\_CONFIG} & \code{scan/configs/lcdm.py} & config module \\
...
\code{PA\_SHARD} & \code{auto} & shard the batch axis when $>1$ GPU (\code{auto}/\code{0}/\code{1}) \\
...
\code{SMC\_NUIS\_BOX\_NSIG} & $6.0$ & unbounded-nuisance box half-width ($\times$scale; full plik) \\
\code{SMC\_TAU\_FLOOR} & $0.01$ & $\tau$ lower-edge floor \\
\code{SMC\_LMAX} & $2508$ & theory $\Lmax$ \\
\code{SMC\_RTOL} & $10^{-5}$ & perturbation tolerance \\
\code{SMC\_USE\_LOWTT} & config (True) & include low-$\ell$ TT \\
\code{SMC\_USE\_LOWEE} & config (True) & include low-$\ell$ EE \\
\code{SMC\_RESUME} & \texttt{""} & resume checkpoint path (auto-detects \code{<out>\_state.npz}) \\
